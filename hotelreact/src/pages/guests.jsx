...
const Guests = () => {
  ...
  const handleCheckOutClick = () => {
    const checkedOutGuests = guests.filter((guest) =>
      guest.reservations.every(
        (reservation) => new Date(reservation.check_out_date) < new Date()
      )
    );
  ...
  const currentGuests = displayedGuests.slice(
    indexOfFirstGuest,
    indexOfLastGuest
  );
        ...
        <div className="content">
          <header>
            <div className="header">
              <button className="tab" onClick={handleAllClick}>
                All
              </button>
              <button className="tab" onClick={handleCheckInClick}>
                Check In
              </button>
              <button className="tab" onClick={handleCheckOutClick}>
                Check Out
              </button>
            </div>
            <button className="add" onClick={handleAddGuestClick}>
              Add Guest
            </button>
          </header>
          <table>
            ...
            <tbody>
              {currentGuests.map((guest) => {
                ...
                const room = activeReservation
                  ? rooms.find((room) => room.id === activeReservation.room_id)
                  : null;
                ...
                return (
                  <tr key={guest.id}>
                    ...
                    <td>{room ? `Room ${room.room_number}` : "No Room"}</td>
                    <td>
                      <button
                        className="edit-btn" style={{ marginBottom: "8px" }}
                        onClick={() =>
                          navigate(`/addguest/${guest.id}`, {
                            state: { guest },
                          })
                        }
                      >
          ...
          <div className="pagination">
            ...
            <span>
              Page {currentPage} of{" "}
              {Math.ceil(displayedGuests.length / guestsPerPage)}
            </span>
            <button
              className="tab"
              onClick={() => setCurrentPage(currentPage + 1)}
              disabled={indexOfLastGuest >= displayedGuests.length}
            >
              Next
            </button>
          </div>
